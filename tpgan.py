...
from tensorflow.keras.layers import Input, Add, Maximum, Dense, Activation, BatchNormalization, Conv2D, Conv2DTranspose, Reshape, Flatten, Concatenate, Lambda, MaxPooling2D, ZeroPadding2D, Dropout, AveragePooling2D, Average
from tensorflow.keras.optimizers import SGD, Adam
from tensorflow.keras.layers import LeakyReLU
from tensorflow.keras.callbacks import TensorBoard, Callback
from tensorflow.keras.models import Model, Sequential, load_model
from tensorflow.keras import regularizers
from tensorflow.keras.initializers import Constant, RandomNormal, TruncatedNormal, Zeros
...
class TPGAN():
    ...
    def train_generator(self, train_gen, valid_gen, optimizer=Adam(lr=0.0001), steps_per_epoch=100, epochs=1, is_output_img=False, out_dir='D:/desktop/tpgan_keras/keras_tpgan/out/', out_period=1,
        ...
        def _loss_ip(y_true, y_pred):
            ...
            """
            '''
            y_true_gray = Lambda(lambda x: tf.image.rgb_to_grayscale(x), output_shape=(multipie_gen.IMG_H, multipie_gen.IMG_W, 3))(y_true)
            ...
            '''
            embedder = FaceNet()
            #y_true_t = tf.convert_to_tensor(y_true, tf.float32, name='y_true_t')
            session = tf.Session()
            y_true_n = y_true.eval(session=session)
            y_true_u = y_true_n.astype(np.uint8)
            y_true_dis = embedder(y_true_u)
            
            #y_pred_t = tf.convert_to_tensor(y_pred, tf.float32, name='y_pred_t')
            
            y_pred_n = y_pred.eval(session=session)
            y_pred_u = y_pred_n.astype(np.uint8)
            y_pred_dis = embedder(y_pred_u)
            
            
            return lambda_ip * K.mean(K.abs(y_pred_dis - y_true_dis))
